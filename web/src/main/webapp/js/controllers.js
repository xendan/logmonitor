...
//Must be globall to clear events on page reload
var refreshTimeoutVars = {};
function showDialog(content, title, yesHandler) {
    $('<div></div>').appendTo('body').html(content).dialog({
        modal: true,
        title: title,
        zIndex: 10000,
        autoOpen: true,
...
function AllConfigsController($scope, Configs, LogEntries, $routeParams) {
    ...
    var getLoadEntriesHandler = function (matcher, envId) {
        return function (items) {
            ...
            var counter = 0;
            eachEntry($scope.entries[envId][matcher.id], function (entry) {
                counter++;
            });
    ...
    $scope.deleteEntriesInEnv = function (env) {
        ...
        showDialog(createContent(), "Delete all entries", function () {
    ...
    $scope.createMatchConfig = function(group) {
        $.get("partials/matcher.html", function(data) {
